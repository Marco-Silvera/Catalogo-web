...
export async function generateStaticParams() {
    const perfumes = await fetchPerfumesByVersion("Sellado");
    return perfumes.map((perfume) => ({ path: perfume.path }));
}

export default async function PerfumePage({ params }) {
    const perfume = await getPerfumeByPath(params.path, "Sellado");

    if (!perfume) {
        return <h1 className="text-center py-10">Perfume no encontrado</h1>;
    }
