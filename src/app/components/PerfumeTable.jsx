...
function PerfumeTable({ perfumes, onEdit, onDelete, loading }) {
    const [filteredPerfumes, setFilteredPerfumes] = useState(null);
    const [filter, setFilter] = useState("added");
    const [searchTerm, setSearchTerm] = useState("");

    // Cada vez que cambien los perfumes, el filtro o la búsqueda se recalcula
    useEffect(() => {
        if (!loading) {
            handleFilter();
        }
    }, [filter, searchTerm, perfumes, loading]);
    ...
    const handleFilter = () => {
        ...
        switch (filter) {
            ...
            default:
                ...
                sortedPerfumes = sortedPerfumes.reverse();
                break;
        }
        ...
        <section>
            ...
            <div className="flex justify-between gap-5 flex-col sm:flex-row sm:items-center mb-4">
                ...
                <span className="flex sm:justify-center self-center order-2 sm:order-3 text-xs sm:text-sm md:text-base">
                    {filteredPerfumes ? filteredPerfumes.length : 0} productos
                </span>
